...
SCI.AI 全球供应链新闻智能编辑系统是一套**全自动化的供应链行业新闻生产流水线**。基于 [OpenClaw](https://github.com/openclaw/openclaw) 开源框架，深度定制了多 Agent 协作、内容安全审核、多渠道分发等能力，实现从情报采集 → 内容生成 → 质量审核 → 多平台发布的端到端自动化。
...
│  @EvoChain_LinX_bot · @Mia_Ling_bot · @linzihan_bot        │
└─────────────────────────────────────────────────────────────┘
```
